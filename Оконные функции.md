...
Условия N ```PRECEDING``` и N ```FOLLOWING``` применимы исключительно к числовым столбцам и столбцам с датами. Однако, разные СУБД используют свой собственный синтаксис для работы с датами в диапазонных фреймах.  
<br>  
Например, в PostgreSQL используется следующий формат:
```RANGE BETWEEN INTERVAL '1 day' PRECEDING AND INTERVAL '10 days' FOLLOWING```
<br>
А в MySQL синтаксис выглядит так:
```RANGE BETWEEN INTERVAL 1 DAY PRECEDING AND INTERVAL 10 DAY FOLLOWING```
