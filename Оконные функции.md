...
Так почему же функции ```LAST_VALUE``` / ```FIRST_VALUE``` могут отображать информацию некорректно? Все просто, дело в том, что эти функции работают не только с партицией внутри окна, но также и с фреймом внутри патриции. Партиция зафиксирована а вот фрейм зависит от положения текущий записи внутри партиции и постоянно меняется.   
```FIRST_VALUE``` - возвращает первую строчку фрейма, которая совпадает с началом партиции. Именно благодаря этому функции отрабатывает как положено.  
```LAST_VALUE``` - чтобы функция отрабатывала как положено, мы должны явно задать границы фрейма внутри партиции. Благо делается это элементарно, нужно всего лишь добавить ```rows between unbounded preceding and unbounded following```.
